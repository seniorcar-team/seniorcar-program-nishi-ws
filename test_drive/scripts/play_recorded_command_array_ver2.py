...
class playRecordedorder:
	...
	filename_pre = '/home/nishi/catkin_ws/src/test_drive/command_array'
	file_num = input('>> ')
	file_type = '.txt'
        filename = filename_pre+file_num+file_type
    ...
    def update_order(self):
        ...
        if len(self.speed_array) -1 > self.index:
            ...
            if self.speed_array[self.index]*3.6 > 2.0:
                ...
                self.seniorcar_command.accel_opening = 127*(self.speed_array[self.index]*3.6)/2.0#self.speed_array[self.index]*3.6/2*127
