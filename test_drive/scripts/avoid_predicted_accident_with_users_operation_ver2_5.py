...
import rospy
import numpy as np# add in 2017_11_9
...
MAX_CHANGE_STEER_ANGLE_BY_STEP = 3.0 # １ステップで変化できる舵角の最大角度。いきなり舵がふられるのを防ぐ
				...
				if avoid_index == -1:
					...
					if avoid_index < 6 and avoid_index > 1:#to adjust offset.11/6 
						...
						if self.t1_flag.data == True:
							self.t1 = rospy.get_time()#probably it doesn't wok
				...
				if self.t1_flag.data == False:
					...
					if self.t3 < DELAY_TIME and self.t3 > 0 or self.t3 == DELAY_TIME : # I will check here .11/7_9_28
						...
						if self.t1_flag.data == True:
							print ("True")
						#self.func()
					elif self.t3 > DELAY_TIME and self.t3 < DELAY_TIME*4:#from 4 to 3
